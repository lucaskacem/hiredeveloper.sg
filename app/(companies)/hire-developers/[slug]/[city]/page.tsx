...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const titles = showYear ? [
    `Hire ${tech} in ${city} - ${year} | HireDeveloper.sg`,
    `${tech} in ${city} — Top 2% Vetted Talent ${year} | HireDeveloper.sg`,
    `Hire Freelance ${tech} in ${city} - ${year} | HireDeveloper.sg`,
    `Best ${tech} in ${city}, ${country} - ${year} | HireDeveloper.sg`,
    `Find ${tech} in ${city} — Matched in 48h | HireDeveloper.sg`,
    `${city} ${tech} for Hire - ${year} | HireDeveloper.sg`,
    `Hire Remote ${tech} in ${city} - ${year} | HireDeveloper.sg`,
  ] : [
    `Hire ${tech} in ${city} | HireDeveloper.sg`,
    `${tech} in ${city} — Top 2% Vetted Talent | HireDeveloper.sg`,
    `Hire Freelance ${tech} in ${city} | HireDeveloper.sg`,
    `Best ${tech} in ${city}, ${country} | HireDeveloper.sg`,
    `Find ${tech} in ${city} — Matched in 48h | HireDeveloper.sg`,
    `${city} ${tech} for Hire | HireDeveloper.sg`,
    `Hire Remote ${tech} in ${city} | HireDeveloper.sg`,
  ];
  ...
  return {
    ...
    alternates: {
      canonical: `https://hiredeveloper.sg/hire-developers/${slug}/${citySlug}`,
    },
  };
}
...
export default async function TechCityPage({ params }: Props) {
  ...
  const jsonLdService = {
    ...
    url: `https://hiredeveloper.sg/hire-developers/${slug}/${city.slug}`,
    provider: {
      '@type': 'Organization',
  ...
  const faqItems = [
    {
      ...
      a: `With HireDeveloper.sg, you can access vetted ${sub.name} in ${city.name} within 48 hours. Our platform provides access to the top 2% of ${sub.name} specializing in ${sub.skills.slice(0, 3).join(', ')} and other technologies.`,
    },
    {
    ...
    },
    {
      ...
      a: `Yes, all ${sub.name} on HireDeveloper.sg specialize in remote work. Developers based in ${city.name} operate in the UTC+8 timezone, are fluent in English, and have experience working with distributed teams and modern collaboration tools.`,
    },
    {
      q: `How quickly can I hire a ${sub.name.replace(' Developers', '').replace(' Engineers', '')} specialist in ${city.name}?`,
      a: `With HireDeveloper.sg, you receive vetted ${sub.name} profiles from ${city.name} within 48 hours. After the interview process, you can often hire the developer within a week and start collaborating immediately.`,
    },
  ];
  ...
  const jsonLdBreadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Hire Developers', item: 'https://hiredeveloper.sg/hire-developers' },
      { '@type': 'ListItem', position: 3, name: sub.name, item: `https://hiredeveloper.sg/hire-developers/${slug}` },
      { '@type': 'ListItem', position: 4, name: city.name, item: `https://hiredeveloper.sg/hire-developers/${slug}/${city.slug}` },
    ],
  };
    ...
    ? [
        {
          ...
          text: `With a population of ${populationFormatted}, ${city.name} is one of the largest cities in Southeast Asia region. The city is home to numerous tech companies and startups leveraging ${sub.skills.slice(0, 3).join(', ')} \u2014 an ideal breeding ground for top-tier ${sub.name}.`,
        },
        {

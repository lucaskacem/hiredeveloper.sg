...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: showYear ? `Hire ${data.name} in Singapore - ${year} | HireDeveloper.sg` : `Hire ${data.name} in Singapore | HireDeveloper.sg`,
    description: showYear ? `${data.metaDescription} Updated for ${year}.` : data.metaDescription,
    robots: { index: true, follow: true },
    alternates: {
      canonical: `https://hiredeveloper.sg/hire-developers/${slug}`,
    },
  };
}
...
export default async function DeveloperSubcategoryPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    url: `https://hiredeveloper.sg/hire-developers/${slug}`,
    provider: {
      '@type': 'Organization',
  ...
  const jsonLdBreadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: data.name, item: `https://hiredeveloper.sg/hire-developers/${slug}` },
    ],
  };

...
// Danh sách tất cả các pages từ kết quả trước
const allPages = [
  "https://arc.dev/",
...
function createFolderStructure(url) {
...
function extractAssets(html, baseUrl) {
  ...
  function normalizeUrl(src) {
    ...
    if (src.includes('arc.dev') || src.includes('cdn.arc.dev')) {
      // Remove query params for deduplication but keep them in the URL
      return src.split('?')[0] + (src.includes('?') ? '?' + src.split('?')[1] : '');
    }
    return null;
  }
  ...
  while ((match = imgRegex.exec(html)) !== null) {
    ...
    if (normalized) assets.images.add(normalized);
  }
  ...
  while ((match = videoRegex.exec(html)) !== null) {
    const normalized = normalizeUrl(match[1]);
    if (normalized) assets.videos.add(normalized);
  }
  ...
  // Extract from Next.js Image component data
  const nextImageRegex = /<img[^>]+src=["']([^"']+)["'][^>]*>/gi;
  ...
  while ((match = scriptRegex.exec(html)) !== null) {
    ...
    while ((urlMatch = urlRegex.exec(scriptContent)) !== null) {
      ...
      if (normalized) {
        if (normalized.match(/\.(mp4|webm|mov)$/i)) {
          assets.videos.add(normalized);
        } else {
          assets.images.add(normalized);
        }
      }
    }
  }
...
// Process single page
async function processPage(pageUrl, index, total) {
  try {
    console.log(`\n[${index + 1}/${total}] Processing: ${pageUrl}`);
    
    const html = await fetchPage(pageUrl);
    const assets = extractAssets(html, pageUrl);
    
    const folderPath = createFolderStructure(pageUrl);
    const pageFolder = path.join(folderPath, 'assets');
    ...
    for (const imgUrl of assets.images) {
      try {
        ...
        const filePath = path.join(pageFolder, 'images', fileName);
    ...
    for (const videoUrl of assets.videos) {
      try {
        ...
        const filePath = path.join(pageFolder, 'videos', fileName);
        fs.mkdirSync(path.dirname(filePath), { recursive: true });
    ...
    console.log(`✅ Completed: ${assets.images.length} images, ${assets.videos.length} videos`);
    
  } catch (err) {
    console.error(`❌ Error processing ${pageUrl}:`, err.message);
    failedDownloads.push({ url: pageUrl, type: 'page', error: err.message });
  }
}

// Main function
async function main() {
  console.log(`🚀 Starting to download assets from ${allPages.length} pages...\n`);
  
  // Create base directory
  fs.mkdirSync(BASE_DIR, { recursive: true });
  
  // Process pages with concurrency limit (reduced to avoid rate limiting)
  const concurrency = 2;
  for (let i = 0; i < allPages.length; i += concurrency) {
    const batch = allPages.slice(i, i + concurrency);
    await Promise.all(batch.map((page, idx) => processPage(page, i + idx, allPages.length)));
  ...
  const summary = {
    ...
    downloadedAssets: downloadedAssets.size,
    failedDownloads: failedDownloads,
    timestamp: new Date().toISOString()
  };
  
  fs.writeFileSync(
    path.join(BASE_DIR, 'download-summary.json'),
  ...
  console.log(`📁 Assets saved to: ${BASE_DIR}`);
}
